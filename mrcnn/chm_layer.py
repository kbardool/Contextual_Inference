...
"""
##----------------------------------------------------------------------------------------------------------------------          
## Inference methods moved to CHMLayerInference
##----------------------------------------------------------------------------------------------------------------------          
...
def build_heatmap_inference(in_tensor, config, names = None):
    ...
    gauss_heatmap    = KB.identity(KB.reshape(gauss_norm,  heatmap_shape ) , name = names[0]+'_norm')   

    print('   gauss_flatten    : ', KB.int_shape(gauss_flatten) , gauss_flatten.get_shape(),' Keras tensor ', KB.is_keras_tensor(gauss_flatten) )
    print('   gauss_heatmap1      : ', KB.int_shape(gauss_norm)   , gauss_norm.get_shape(),' Keras tensor ', KB.is_keras_tensor(gauss_norm) )
    print('   gauss_heatmap final : ', KB.int_shape(gauss_heatmap)    , gauss_heatmap.get_shape(),' Keras tensor ', KB.is_keras_tensor(gauss_heatmap) )
    print('    complete')
    ...
    return  gauss_sum, gauss_heatmap    # [gauss_sum, gauss_scatt, means, covar]
...
class CHMLayerInference(KE.Layer):
    ...
    def call(self, inputs):
        ...
        pred_tensor , _  = build_predictions(mrcnn_class, mrcnn_bbox, output_rois, self.config)
